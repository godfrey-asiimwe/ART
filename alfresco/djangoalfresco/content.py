...
def get_content(nodeId, token):
  ...
  try:
    response = requests.get(settings.URL_CORE + settings.URL_NODES + "/" + nodeId + "/" + default_params,
                            headers=headers)
    return response
  except:
    print("Error when querying the Alfresco API.")
...
def post_node_children(nodeId, nameDocument, token):
  ...
  headers = {'Accept': 'application/json', 'Content-Type': 'application/json',
             'Authorization': auth + base64.b64encode(bytes(token, "utf-8"))}
  default_params = "children"
  body = '{ "name" : "' + nameDocument + '" , "nodeType": "cm:content" }'

  try:
    response = requests.post(settings.URL_CORE + settings.URL_NODES + "/" + nodeId + "/" + default_params,
...
def put_content_node(nodeId, path, token):
  ...
  try:
    response = requests.put(settings.URL_CORE + settings.URL_NODES + "/" + nodeId + "/" + default_params,
