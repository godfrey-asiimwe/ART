...
def post_ticket(username, password):
  ...
  try:
    response = requests.post(settings.URL_AUTHENTICATION + settings.URL_TICKETS, headers=headers, data=body,
                             auth=HTTPBasicAuth(settings.USER_LOGIN, settings.USER_PASSWORD))
